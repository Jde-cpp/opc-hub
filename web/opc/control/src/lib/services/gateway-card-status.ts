...
import { CardStatusValue, counted, countRows, ICardStatus, plural, queryRows } from 'jde-framework';
...
export class GatewayCardStatus implements ICardStatus{
	...
	async status():Promise<CardStatusValue>{
		...
		const unreachable = results.filter( r=>r.status=='rejected' ).length;
		const notAnswering = `${counted(unreachable, 'gateway')} not answering`;
		const detail = !gateways.length ? 'no gateway registered' : unreachable ? notAnswering : `on ${counted(gateways.length, 'gateway')}`;
		const summary = !gateways.length ? 'No gateway registered'
			: `${counted(connections, 'OPC connection')} on ${counted(gateways.length, 'gateway')}` + (unreachable ? ` · ${notAnswering}` : '');
