...
# whose, "<SID> installed <root>" or "<SID> owns <path>";  12 (AllUsers) a link another account made;  13 the probe itself
# failed.  Setup reads the output with nsExec /OEM (#50).  A SID, not a name:  the pipe is in the console's OEM code page, which
# best-fit an account outside it (an L-stroke read as L) - so Setup, which is Unicode, looks the name up itself (#62).
param(
